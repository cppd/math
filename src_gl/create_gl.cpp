...
void write_data(const std::vector<FuncData>& data)
{
        ...
        std::ofstream f_proc_header(std::string(GL_CPP_NAME) + "_proc.h");
        std::string proc_header_guard(to_upper(GL_CPP_NAME) + "_PROC_H");
        std::ofstream f_proc_impl(std::string(GL_CPP_NAME) + "_proc.cpp");

        std::ofstream f_header(std::string(GL_CPP_NAME) + ".h");
        std::string header_guard(to_upper(GL_CPP_NAME) + "_H");
        std::ofstream f_impl(std::string(GL_CPP_NAME) + ".cpp");
        ...
        f_proc_header << "#ifndef " << proc_header_guard << "\n";
        f_proc_header << "#define " << proc_header_guard << "\n";
        f_proc_header << "\n";
        ...
        f_proc_impl << "\n";
        f_proc_impl << "#include \"" << GL_CPP_NAME << "_proc.h\"\n";
        f_proc_impl << "\n";
        ...
        f_proc_impl << INDENT << INDENT << "ptr = reinterpret_cast<" << GL_CPP_NAME << "::PTR>(GetProcAddress(module, str));\n";
        ...
        // gl_cpp.h первая часть с указателями

        f_header << "// Generated from glcorearb.h\n";
        f_header << "\n";
        f_header << "#pragma once\n";
        f_header << "#ifndef " << header_guard << "\n";
        f_header << "#define " << header_guard << "\n";
        f_header << "\n";
        ...
        // gl_cpp.h вторая часть с функциями
        ...
        for (const FuncData& s : data)
        {
                ...
                f_header << "{\n";
                f_header << INDENT << s.return_statement << GL_CPP_NAME << "::" << s.pointer_name << "(" << s.parameter_names
                         << ");\n";
                f_header << ("}\n");
        }
        f_header << "\n";
        ...
        // gl_cpp.cpp первая часть с функцией и указателями

        f_impl << "// Generated from glcorearb.h\n";
        f_impl << "\n";
        f_impl << "#include \"" << GL_CPP_NAME << ".h\"\n";
        f_impl << "\n";
        f_impl << "#include \"" << GL_CPP_NAME << "_proc.h\"\n";
        f_impl << "\n";
        ...
        for (const FuncData& s : data)
        {
                ...
                f_impl << s.pointer_type << padding << " " << GL_CPP_NAME << "::" << s.pointer_name << ";\n";
        }

        // gl_cpp.cpp вторая часть с определением адресов функций

        f_impl << "\n";
        f_impl << "void " << GL_CPP_NAME << "::init()\n";
        f_impl << "{\n";
        for (const FuncData& s : data)
        {
                std::string padding = std::string(pointer_name_length - s.pointer_name.size(), ' ');

                f_impl << INDENT << "set(&" << GL_CPP_NAME << "::" << s.pointer_name << ", " << padding << "\"" << s.function_name
                       << "\");\n";
        }
        f_impl << "}\n";
        f_impl << "\n";
