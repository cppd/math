...
DebugMemory::DebugMemory(const vulkan::Device& device)
...
VkDescriptorSetLayout DebugMemory::descriptor_set_layout() const noexcept
{
        return m_descriptor_set_layout;
}

VkDescriptorSet DebugMemory::descriptor_set() const noexcept
{
        return m_descriptor_set;
}

void DebugMemory::set_object_image(const vulkan::StorageImage& storage_image) const
{
...
void DebugMemory::set_points(const vulkan::BufferWithHostVisibleMemory& buffer) const
{
...
void DebugMemory::set_point_count(const vulkan::BufferWithHostVisibleMemory& buffer) const
{
        ASSERT(buffer.usage(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT));
...
void DebugConstant::set_local_size_y(uint32_t v)
{
        static_assert(std::is_same_v<decltype(m_data.local_size_y), decltype(v)>);
        m_data.local_size_y = v;
}

void DebugConstant::set_local_size_z(uint32_t v)
{
...
size_t DebugConstant::size() const noexcept
{
