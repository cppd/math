...
{
OpticalFlowGrayscaleMemory::OpticalFlowGrayscaleMemory(const opengl::Texture& src, const opengl::Texture& dst)
        : m_buffer(sizeof(Images), GL_MAP_WRITE_BIT)
{
        ASSERT(dst.format() == GL_R32F);

        Images images;

        images.src = src.texture_handle();
        images.dst = dst.image_handle_write_only();
