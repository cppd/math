...
void RendererImpl::info(info::Functionality* const functionality) const
{
        functionality->shadow_zoom = !ray_tracing_;
}

void RendererImpl::info(info::Description* const description) const
{
...
void RendererImpl::create_transparency_buffers()
{
...
void RendererImpl::create_opacity_buffers()
{
        opacity_buffers_.create_buffers(
                *device_, {graphics_queue_->family_index()}, render_buffers_->sample_count(), render_buffers_->width(),
                render_buffers_->height());
}

void RendererImpl::delete_mesh_shadow_mapping_buffers()
{
        if (ray_tracing_)
        {
                return;
...
void RendererImpl::create_mesh_shadow_mapping_buffers()
{
        if (ray_tracing_)
        {
                return;
...
void RendererImpl::create_mesh_render_command_buffers()
{
...
void RendererImpl::create_mesh_shadow_mapping_command_buffers()
{
        if (ray_tracing_)
        {
                return;
...
void RendererImpl::create_mesh_command_buffers()
{
        create_mesh_render_command_buffers();
        create_mesh_shadow_mapping_command_buffers();
}

void RendererImpl::create_volume_command_buffers()
{
        ...
        const auto copy_depth = [this](const VkCommandBuffer command_buffer)
        {
                ...
                render_buffers_->commands_depth_copy(
                        command_buffer, depth_copy_image_->image().handle(), DEPTH_COPY_IMAGE_LAYOUT, viewport_, INDEX);
        };
...
void RendererImpl::set_volume_matrix()
{
        if (ray_tracing_)
        {
...
void RendererImpl::acceleration_structure_create()
{
        if (!ray_tracing_)
        {
                return;
...
void RendererImpl::acceleration_structure_update_matrices() const
{
        if (!ray_tracing_)
        {
                return;
...
std::unique_ptr<MeshObject> RendererImpl::mesh_create()
{
...
void RendererImpl::mesh_visibility_changed()
{
        create_mesh_command_buffers();
        acceleration_structure_create();
}

void RendererImpl::mesh_visible_changed(const MeshObject::UpdateChanges& update_changes)
{
...
std::unique_ptr<VolumeObject> RendererImpl::volume_create()
{
        return create_volume_object(
                ray_tracing_, device_, {graphics_queue_->family_index()}, transfer_command_pool_, transfer_queue_,
                volume_image_layouts_, volume_renderer_.image_sampler(), volume_renderer_.transfer_function_sampler());
}

void RendererImpl::volume_visibility_changed()
{
        create_volume_command_buffers();
        set_volume_matrix();
}

void RendererImpl::volume_visible_changed(const VolumeObject::UpdateChanges& update_changes)
{
...
void RendererImpl::view_show_normals_changed()
{
        create_mesh_render_command_buffers();
}

void RendererImpl::view_shadow_zoom_changed()
{
        create_mesh_shadow_mapping_buffers();
        create_mesh_command_buffers();
        create_volume_command_buffers();
...
void RendererImpl::view_matrices_changed()
{
        set_volume_matrix();
        if (ray_tracing_)
        {
        ...
        else
        {
                ...
                volume_renderer_.set_matrix(renderer_view_.vp_matrix(), renderer_view_.world_to_shadow_matrix());
        }
}

void RendererImpl::view_clip_plane_changed(const bool visibility_changed)
{
...
void RendererImpl::view_show_clip_plane_lines_changed()
{
...
RendererImpl::RendererImpl(
        ...
        : device_(device),
          ...
          ggx_f1_albedo_(*device_, {graphics_queue_->family_index()}, *transfer_command_pool_, *transfer_queue_),
          ...
          renderer_draw_(device_->handle(), transparency_buffers_.buffer_size(), &mesh_renderer_, &volume_renderer_)
{
        if (ray_tracing_)
        {
