...
template <typename T>
T g_integral(T w1, T w2, T m, T t)
{
        // sqrt(2)
        ...
        const T ts = t / SQRT_2;
        return (SQRT_PI_2 / t) * (std::erf(ts * (w2 - m)) - std::erf(ts * (w1 - m)));
}

template <typename T>
T g_integral(T w1, T w2, T m, T t1, T t2)
{
        ASSERT(w1 < w2);
        if (w2 <= m)
        {
                return g_integral(w1, w2, m, t1);
        }
        if (w1 >= m)
        {
                return g_integral(w1, w2, m, t2);
        }
        return g_integral(w1, m, m, t1) + g_integral(m, w2, m, t2);
}
...
template <typename T>
T x_31_integral(T w1, T w2)
{
        static_assert(std::is_floating_point_v<T>);

        T g1 = g_integral(w1, w2, T(442.0), T(0.0624), T(0.0374));
        T g2 = g_integral(w1, w2, T(599.8), T(0.0264), T(0.0323));
        T g3 = g_integral(w1, w2, T(501.1), T(0.0490), T(0.0382));
        return T(0.362) * g1 + T(1.056) * g2 + T(-0.065) * g3;
}
...
template <typename T>
T y_31_integral(T w1, T w2)
{
        static_assert(std::is_floating_point_v<T>);

        T g1 = g_integral(w1, w2, T(568.8), T(0.0213), T(0.0247));
        T g2 = g_integral(w1, w2, T(530.9), T(0.0613), T(0.0322));
        return T(0.821) * g1 + T(0.286) * g2;
}
...
template <typename T>
T z_31_integral(T w1, T w2)
{
        static_assert(std::is_floating_point_v<T>);

        T g1 = g_integral(w1, w2, T(437.0), T(0.0845), T(0.0278));
        T g2 = g_integral(w1, w2, T(459.0), T(0.0385), T(0.0725));
        return T(1.217) * g1 + T(0.681) * g2;
}
...
template <XYZ xyz, typename T>
std::enable_if_t<xyz == XYZ_31, T> cie_x_integral(T w1, T w2)
{
        return xyz_implementation::x_31_integral(w1, w2);
}
template <XYZ xyz, typename T>
std::enable_if_t<xyz == XYZ_31, T> cie_y_integral(T w1, T w2)
{
        return xyz_implementation::y_31_integral(w1, w2);
}
template <XYZ xyz, typename T>
std::enable_if_t<xyz == XYZ_31, T> cie_z_integral(T w1, T w2)
{
        return xyz_implementation::z_31_integral(w1, w2);
}
