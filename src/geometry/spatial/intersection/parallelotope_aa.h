...
namespace ns::geometry::spatial::intersection::parallelotope_aa
{
...
ParallelotopeAA<N, T> create_random_parallelotope(RandomEngine& engine)
{
        constexpr T ORG_INTERVAL = 10;
        constexpr T MIN_LENGTH = 0.1;
        constexpr T MAX_LENGTH = 10;

        return ParallelotopeAA<N, T>(
                random::point<N, T>(ORG_INTERVAL, engine), random::aa_vectors<N, T>(MIN_LENGTH, MAX_LENGTH, engine));
}

template <std::size_t N, typename T, typename RandomEngine>
std::vector<numerical::Ray<N, T>> create_rays(
        const ParallelotopeAA<N, T>& p,
...
void check_intersection_count(const ParallelotopeAA<N, T>& p, const std::vector<numerical::Ray<N, T>>& rays)
{
        if (!(rays.size() % 3 == 0))
        {
...
{
        const ParallelotopeAA<N, T> parallelotope = create_random_parallelotope<N, T>(engine);
...
void test_intersection()
{
        ...
        const ParallelotopeAA<N, T> p = create_random_parallelotope<N, T>(engine);
