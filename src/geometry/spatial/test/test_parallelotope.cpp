...
template <>
constexpr double EQUALITY_EPSILON<float> = 1e-5;
template <>
constexpr double EQUALITY_EPSILON<double> = 1e-14;
...
double compute_intersections_per_second(const int point_count, std::mt19937_64& engine)
{
        ...
        const Parallelotope<N, T> parallelotope(
                generate_org<N, T>(ORG_INTERVAL, engine), generate_vectors<N, N, T>(MIN_LENGTH, MAX_LENGTH, engine));
        const std::vector<Ray<N, T>> rays = rays_for_intersections(parallelotope, point_count, engine);
...
void test_performance()
{
        ...
        LOG("<" + to_string(N) + ", " + type_name<T>() + ">, #1 = " + to_string_digit_groups(std::llround(performance))
