...
T HyperplaneParallelotope<N, T>::intersection_cost()
{
        static const T cost = []
        {
                const double p = spatial::testing::hyperplane_parallelotope::compute_intersections_per_second<N, T>();
...
#define FUNCTION_INSTANTIATION_N_T(N, T) template T HyperplaneParallelotope<(N), T>::intersection_cost();
