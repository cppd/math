...
template <std::size_t N, typename T>
std::vector<Simplex<N, T>> divide_simplices(unsigned min_count, std::vector<Simplex<N, T>> simplices)
{
        if (simplices.empty())
        {
                return {};
        ...
        while (simplices.size() < min_count)
        {
                ...
                divide_simplex(*iter++, &tmp);
                tmp.reserve(simplices.size() * tmp.size());
                for (; iter != simplices.end(); ++iter)
                {
                        divide_simplex(*iter, &tmp);
                }
...
std::vector<Simplex<N, T>> create_sphere(unsigned facet_min_count)
{
        ...
        return divide_simplices(facet_min_count, std::move(simplices));
}
