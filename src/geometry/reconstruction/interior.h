...
{
namespace interior_implementation
{
// Definition 5.4 (i)
...
std::vector<bool> find_interior_vertices(
        ...
        const std::vector<ManifoldVertex<N>>& vertices)
{
        namespace impl = interior_implementation;

        std::vector<bool> interior_vertices(vertices.size(), false);

        std::size_t interior_count = 0;

        impl::initial_phase(rho, cosine_of_alpha, vertices, &interior_vertices, &interior_count);
        ...
        {
                std::size_t count = 0;
                impl::expansion_phase(rho, cosine_of_alpha, vertices, &interior_vertices, &count);
                if (count == 0)
                {
                        break;
                }
        ...
        const std::vector<bool>& interior_vertices)
{
        ...
        {
                cocone_facets[i] = impl::interior_facet(delaunay_facets, facet_data, interior_vertices, i);
        }
