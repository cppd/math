...
{
        const auto cmp = [&]<std::size_t N>(const numerical::Vector<N, T>& a, const numerical::Vector<N, T>& b)
        {
...

        {
                ...
                cmp(s[3], {-1.17320508075688772934L, 1.98267949192431122707L});
                cmp(s[4], {-1, 1.82766312060385914019L});
        }

        {
                ...
                cmp(s[3], {0.926794919243112270686L, -2.16535898384862245418L});
                cmp(s[4], {1.1L, -2.36970562748477140596L});
        }
}
