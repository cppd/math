...
template <typename T>
void Direction21<T>::save(const TrueData<2, T>& true_data)
{
        if (!nees_)
        {
                nees_.emplace();
...
template <typename T>
void Direction21<T>::reset(const Measurements<2, T>& m)
{
...
template <typename T>
std::optional<UpdateInfo<2, T>> Direction21<T>::update(const Measurements<2, T>& m, const Estimation<T>& estimation)
{
        check_time(m.time);
        ...
        const bool has_angle = angle_p && *angle_p <= angle_estimation_variance_;
...
template <typename T>
std::string Direction21<T>::consistency_string(const std::string& name) const
{
        if (!nees_)
        {
                return {};
