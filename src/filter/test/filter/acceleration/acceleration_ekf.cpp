...
template <typename T>
void AccelerationEkf<T>::save(const TrueData<2, T>& true_data)
{
        if (!nees_)
        {
                nees_.emplace();
...
template <typename T>
std::optional<UpdateInfo<T>> AccelerationEkf<T>::update(const Measurements<2, T>& m, const Estimation<T>& estimation)
{
        check_time(m.time);
