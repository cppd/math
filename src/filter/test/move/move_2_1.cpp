...
template <typename T>
void Move21<T>::save(const T time, const TrueData<2, T>& true_data)
{
...
template <typename T>
void Move21<T>::check_time(const T time) const
{
...
template <typename T>
const std::string& Move21<T>::name() const
{
        return name_;
}

template <typename T>
color::RGB8 Move21<T>::color() const
{
        return color_;
}

template <typename T>
std::string Move21<T>::angle_string() const
{
        std::string s;
        s += name_;
...
template <typename T>
std::string Move21<T>::consistency_string() const
{
        if (!nees_)
        {
                return {};
...
template <typename T>
const std::vector<TimePoint<2, T>>& Move21<T>::positions() const
{
        return positions_;
}

template <typename T>
const std::vector<TimePoint<2, T>>& Move21<T>::positions_p() const
{
        return positions_p_;
}

template <typename T>
const std::vector<TimePoint<1, T>>& Move21<T>::speeds() const
{
        return speeds_;
}

template <typename T>
const std::vector<TimePoint<1, T>>& Move21<T>::speeds_p() const
{
        return speeds_p_;
}

template class Move21<float>;
template class Move21<double>;
template class Move21<long double>;
}
