...
template <std::size_t N, typename T>
void PositionVariance<N, T>::add_nees_checks(const TrueData<N, T>& true_data)
{
...
template <std::size_t N, typename T>
bool PositionVariance<N, T>::prepare_position_variance(const Measurements<N, T>& m)
{
        ASSERT(m.position);
...
template <std::size_t N, typename T>
void PositionVariance<N, T>::update_position_variance(
        const Measurements<N, T>& m,
        const PositionFilterUpdate<N, T>& update)
{
        position_variance_.push(update.residual);
...
void PositionVariance<N, T>::update_position(const Measurements<N, T>& m)
{
        ...
        {
                filter_->reset(m.position->value, VARIANCE_DEFAULT<N, T>);
                last_predict_time_ = m.time;
                last_update_time_ = m.time;
                save_results(m.time);
...
template <std::size_t N, typename T>
[[nodiscard]] Vector<N, T> PositionVariance<N, T>::velocity() const
{
        return filter_->velocity();
}

template <std::size_t N, typename T>
[[nodiscard]] Matrix<N, N, T> PositionVariance<N, T>::velocity_p() const
{
        return filter_->velocity_p();
}

template <std::size_t N, typename T>
Vector<3 * N, T> PositionVariance<N, T>::position_velocity_acceleration() const
{
        return filter_->position_velocity_acceleration();
}

template <std::size_t N, typename T>
Matrix<3 * N, 3 * N, T> PositionVariance<N, T>::position_velocity_acceleration_p() const
{
...
template <std::size_t N, typename T>
const std::vector<Point<N, T>>& PositionVariance<N, T>::positions() const
{
        return positions_;
}

template <std::size_t N, typename T>
const std::vector<Point<N, T>>& PositionVariance<N, T>::positions_p() const
{
        return positions_p_;
}

template <std::size_t N, typename T>
const std::vector<Point<1, T>>& PositionVariance<N, T>::speeds() const
{
        return speeds_;
}

template <std::size_t N, typename T>
const std::vector<Point<1, T>>& PositionVariance<N, T>::speeds_p() const
{
        return speeds_p_;
}
