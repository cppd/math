...
template <typename T>
numerical::Matrix<5, 5, T> p(const numerical::Matrix<4, 4, T>& position_velocity_p, const Init<T>& init)
{
...
template <typename T>
[[nodiscard]] numerical::Vector<5, T> add_x(const numerical::Vector<5, T>& a, const numerical::Vector<5, T>& b)
{
...
template <typename T>
numerical::Vector<5, T> f(const T dt, const numerical::Vector<5, T>& x)
{
        const T px = x[0];
        const T vx = x[1];
        const T py = x[2];
...
template <typename T>
constexpr numerical::Matrix<5, 5, T> q(const T dt, const T position_variance, const T angle_variance)
{
        const T dt_2 = power<2>(dt) / 2;
        const numerical::Matrix<5, 3, T> noise_transition{
                {dt_2,    0,  0},
                {  dt,    0,  0},
                {   0, dt_2,  0},
                {   0,   dt,  0},
