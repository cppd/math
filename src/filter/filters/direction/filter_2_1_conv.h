...
namespace ns::filter::filters::direction::filter_2_1_conv
{
template <typename T>
...
template <typename T>
[[nodiscard]] T angle_speed(const numerical::Vector<8, T>& x)
{
        return x[7];
}

template <typename T>
[[nodiscard]] T angle_speed_p(const numerical::Matrix<8, 8, T>& p)
{
        return p[7, 7];
}
}
