...
namespace ns::filter::filters::position::filter_1_conv
{
...
template <std::size_t N, typename T>
[[nodiscard]] numerical::Vector<N, T> position_velocity(const numerical::Vector<N, T>& x)
{
        return x;
}

template <std::size_t N, typename T>
[[nodiscard]] numerical::Matrix<N, N, T> position_velocity_p(const numerical::Matrix<N, N, T>& p)
{
        return p;
}
}
