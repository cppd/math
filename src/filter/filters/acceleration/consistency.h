...
void update_nees(const Filter& filter, const TrueData<2, T>& true_data, Nees<T>& nees)
{
        ...
        nees.angle.add_1(com::wrap_angle(true_data.angle - filter.angle()), filter.angle_p());
        nees.angle_r.add_1(com::wrap_angle(true_data.angle_r - filter.angle_r()), filter.angle_r_p());
}
