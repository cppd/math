...
public:
        ...
        virtual core::UpdateInfo<6, T> update_position_speed_direction_acceleration(
                const Measurement<2, T>& position,
                const Measurement<1, T>& speed,
        ...
        virtual core::UpdateInfo<5, T> update_position_speed_acceleration(
                const Measurement<2, T>& position,
                const Measurement<1, T>& speed,
                const Measurement<2, T>& acceleration,
                std::optional<T> gate) = 0;

        virtual core::UpdateInfo<5, T> update_position_direction_acceleration(
                const Measurement<2, T>& position,
                const Measurement<1, T>& direction,
                const Measurement<2, T>& acceleration,
                std::optional<T> gate) = 0;
        ...
        virtual core::UpdateInfo<4, T> update_position_acceleration(
                const Measurement<2, T>& position,
                const Measurement<2, T>& acceleration,
                std::optional<T> gate) = 0;

        virtual core::UpdateInfo<4, T> update_speed_direction_acceleration(
                const Measurement<1, T>& speed,
                const Measurement<1, T>& direction,
                const Measurement<2, T>& acceleration,
                std::optional<T> gate) = 0;
        ...
        virtual core::UpdateInfo<3, T> update_direction_acceleration(
                const Measurement<1, T>& direction,
                const Measurement<2, T>& acceleration,
                std::optional<T> gate) = 0;

        virtual core::UpdateInfo<2, T> update_acceleration(
                const Measurement<2, T>& acceleration,
                std::optional<T> gate) = 0;
        ...
        virtual core::UpdateInfo<3, T> update_speed_acceleration(
                const Measurement<1, T>& speed,
                const Measurement<2, T>& acceleration,
                std::optional<T> gate) = 0;
...
template <typename T>
std::unique_ptr<Filter1<T>> create_filter_1(T sigma_points_alpha);
}
