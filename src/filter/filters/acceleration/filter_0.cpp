...
template <typename T>
numerical::Matrix<8, 8, T> p(const numerical::Matrix<6, 6, T>& position_velocity_acceleration_p, const Init<T>& init)
{
...
template <typename T>
[[nodiscard]] Vector<8, T> add_x(const Vector<8, T>& a, const Vector<8, T>& b)
{
        Vector<8, T> res = a + b;
        ...
        const T angle_r_variance)
{
        ...
        const numerical::Matrix<8, 4, T> noise_transition{
                {dt_3,    0,  0,  0},
                {dt_2,    0,  0,  0},
                {  dt,    0,  0,  0},
                {   0, dt_3,  0,  0},
...
template <typename T>
Vector<6, T> position_speed_direction_acceleration_h(const Vector<8, T>& x)
{
        // px = px
        // py = py
        // speed = sqrt(vx*vx + vy*vy)
...
template <typename T>
Vector<5, T> position_speed_acceleration_h(const Vector<8, T>& x)
{
        // px = px
        // py = py
        // speed = sqrt(vx*vx + vy*vy)
...
template <typename T>
Vector<5, T> position_direction_acceleration_h(const Vector<8, T>& x)
{
        // px = px
        // py = py
        // angle = atan(vy, vx) + angle + angle_r
...
template <typename T>
Vector<4, T> position_acceleration_h(const Vector<8, T>& x)
{
        // px = px
        // py = py
        // ax = ax*cos(angle) - ay*sin(angle)
...
template <typename T>
Vector<4, T> speed_direction_acceleration_h(const Vector<8, T>& x)
{
        // speed = sqrt(vx*vx + vy*vy)
        // angle = atan(vy, vx) + angle + angle_r
        // ax = ax*cos(angle) - ay*sin(angle)
...
template <typename T>
Vector<3, T> direction_acceleration_h(const Vector<8, T>& x)
{
        // angle = atan(vy, vx) + angle + angle_r
        // ax = ax*cos(angle) - ay*sin(angle)
        // ay = ax*sin(angle) + ay*cos(angle)
...
template <typename T>
Vector<2, T> acceleration_h(const Vector<8, T>& x)
{
        // ax = ax*cos(angle) - ay*sin(angle)
        // ay = ax*sin(angle) + ay*cos(angle)
        const T ax = x[2];
...
template <typename T>
Vector<3, T> speed_acceleration_h(const Vector<8, T>& x)
{
        // speed = sqrt(vx*vx + vy*vy)
        // ax = ax*cos(angle) - ay*sin(angle)
        // ay = ax*sin(angle) + ay*cos(angle)
...
class Filter final : public Filter0<T>
{
        ...
        {
                filter_.emplace(
                        core::create_sigma_points<8, T>(sigma_points_alpha_), x(position_velocity_acceleration, init),
...
template <typename T>
std::unique_ptr<Filter0<T>> create_filter_0(
...
#define TEMPLATE(T) template std::unique_ptr<Filter0<T>> create_filter_0(T, T, T, T);
