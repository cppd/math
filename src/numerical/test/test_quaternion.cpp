...
void test_constant(const T precision)
{
        ...
        test_equal(
                QuaternionHJ<T, JPL>(T{2} * Vector<3, T>(4, -5, 6).normalized()),
                QuaternionHJ<T, JPL>(
...
{
        {
                const QuaternionHJ<T, JPL> q({T{1} / 10, 0, 0});
        ...
        {
                const QuaternionHJ<T, JPL> q({0, T{1} / 10, 0});
        ...
        {
                const QuaternionHJ<T, JPL> q({0, 0, T{1} / 10});
                const Vector<3, T> v(1, 0, 0);
