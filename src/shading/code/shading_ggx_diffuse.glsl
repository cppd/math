...
{
        float v = 1 + shading_square(n_h) * (alpha_2 - 1);
...
{
        float lv = n_l * sqrt(alpha_2 + shading_square(n_v) * (1 - alpha_2));
        float vl = n_v * sqrt(alpha_2 + shading_square(n_l) * (1 - alpha_2));
        return 0.5 / (lv + vl);
}
...
vec3 shading_diffuse_disney_ws(vec3 f0, vec3 rho_ss, float roughness, float n_l, float n_v, float h_l)
{
        float l = pow(1 - n_l, 5);
        float v = pow(1 - n_v, 5);
        float f_d90 = 2 * roughness * shading_square(h_l);
        float c = (1 + (f_d90 - 1) * l) * (1 + (f_d90 - 1) * v);
...
{
        float alpha = shading_square(roughness);
        float alpha_2 = shading_square(alpha);
...
vec3 shading_ggx_diffuse(float metalness, float roughness, vec3 surface_color, vec3 n, vec3 v, vec3 l)
{
        ...
        vec3 diff = shading_diffuse_disney_ws(f0, rho_ss, roughness, n_l, n_v, h_l);
