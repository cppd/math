...
float shading_ggx(float alpha_2, float n_h)
{
        ...
        return n_h * alpha_2 / (shading_PI * shading_sqr(v));
}
...
{
        float lv = n_l * sqrt(mix(shading_sqr(n_v), 1, alpha_2));
        float vl = n_v * sqrt(mix(shading_sqr(n_l), 1, alpha_2));
        return 0.5 / (lv + vl);
}
...
//        return (1 - f0) * rho_ss * ((21 / (20 * shading_PI)) * l * v);
//}
...
vec3 shading_diffuse_disney_without_subsurface(vec3 rho_ss, float roughness, float n_l, float n_v, float h_l)
{
        float l = pow(1 - n_l, 5);
        float v = pow(1 - n_v, 5);
        float d_90 = 0.5 + 2 * roughness * shading_sqr(h_l);
        float f_d = mix(1, d_90, l) * mix(1, d_90, v);
        return rho_ss * (n_l * n_v * (1 / shading_PI) * f_d);
}
...
//        float l = pow(1 - n_l, 5);
//        float v = pow(1 - n_v, 5);
//        float ss_90 = roughness * shading_sqr(h_l);
//        float d_90 = 0.5 + 2 * ss_90;
//        float f_d = mix(1, d_90, l) * mix(1, d_90, v);
//        float f_ss = (1 / (n_l * n_v) - 0.5) * mix(1, ss_90, l) * mix(1, ss_90, v) + 0.5;
//        return rho_ss * (n_l * n_v * (1 / shading_PI) * mix(f_d, 1.25 * f_ss, k_ss));
//}
...
vec3 shading_ggx_diffuse(float metalness, float roughness, vec3 surface_color, vec3 n, vec3 v, vec3 l)
{
        ...
        vec3 spec = shading_fresnel(f0, h_l) * shading_ggx_g2_combined(alpha_2, n_l, n_v) * shading_ggx(alpha_2, n_h);
        vec3 diff = shading_diffuse_disney_without_subsurface(rho_ss, roughness, n_l, n_v, h_l);
